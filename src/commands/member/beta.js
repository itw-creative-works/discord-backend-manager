      ...
      return interaction.editReply({ content: '', embeds: [response] });
		...
		if (betaTesterStatus.applicationAccepted) {
			...
			response.setColor(config.colors.blue)
				...
				.setDescription(`${config.emojis.betaTester} Hey, **${helpers.displayMember(options.user)}**, we are reviewing your **${Manager.config.brand.name} ${helpers.getPrettyRole('beta')}** application right now:\n\n:pencil: **Applied**: ${new Date(betaTesterStatus.applicationDate).toLocaleDateString()} (${betaTesterStatus.daysAppliedAgo} days ago)\n${config.emojis.active} **Active**: ${options.user.roles.cache.has(config.roles.active) ? 'Yes!' : 'No'}\n${config.emojis.premium} **Premium**: ${options.user.roles.cache.has(config.roles.premium) ? 'Yes!' : 'No'}\n*We are only accepting members who are ${config.emojis.active}${roleMention(config.roles.active)} or ${config.emojis.premium}${roleMention(config.roles.premium)}.*\n\nDo **not** ask staff about the status of your application. If you are **friendly** and **active** in our community then you will be accepted to be a part of the **${Manager.config.brand.name} ${helpers.getPrettyRole('beta')} Team**.\n`);
		} else {
